...
if uploaded_file is not None:  ## If the user has uploaded a file
    ...
    try:
        ...
        pairwise = col3.radio("Pairwise T-test?", ["Yes", "No"])
        ...
        if pairwise == 'No':
            ...
            test_cols = col4.multiselect("Select the 2 columns to be compared", df.columns, max_selections=2)
            test_df = df.loc[:, test_cols]  
            if len (test_cols) > 2 or len(test_cols) < 2:
                st.error(f"{t_type} T-test can only be performed on 2 columns")

            try:
                t_test_tesults = hf.t_tests(test_df, paired=paired)
                st.write(t_test_tesults)
